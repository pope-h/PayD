 ...
 */
import { Keypair, Horizon, Networks, TransactionBuilder, Operation, Asset, AuthRequiredFlag, AuthRevocableFlag, } from '@stellar/stellar-sdk';
...
async function main() {
    ...
    console.log(`  Issuer flags: auth_required=${updatedIssuer.flags.auth_required}, auth_revocable=${updatedIssuer.flags.auth_revocable}`);
    ...
    const finalDistribution = await server.loadAccount(distributionKeypair.publicKey());
    const orgUsdBalance = finalDistribution.balances.find((b) => b.asset_code === 'ORGUSD');
...
//# sourceMappingURL=issue-orgusd.js.map