...
describe('StellarService', () => {
  ...
  describe('Transaction Construction', () => {
    ...
    it('should build transaction with multiple operations', async () => {
      ...
      const builder = await StellarService.buildTransaction(testKeypair.publicKey(), operations);
  ...
  describe('Multi-Signature Scenarios', () => {
    ...
    it('should add a signer to an account', async () => {
      ...
      const transaction = await StellarService.addSigner(testKeypair, newSigner.publicKey(), 1);
    ...
    it('should add a signer with custom weight', async () => {
      const newSigner = Keypair.random();
      const transaction = await StellarService.addSigner(testKeypair, newSigner.publicKey(), 5);

      expect(transaction).toBeDefined();
    });

    it('should set account thresholds', async () => {
      const transaction = await StellarService.setAccountThresholds(testKeypair, {
        low: 1,
        med: 2,
        high: 3,
        masterWeight: 1,
  ...
  describe('Sequence Number Edge Cases', () => {
    ...
    it('should throw error when account does not exist', async () => {
      ...
      await expect(StellarService.getSequenceNumber(testKeypair.publicKey())).rejects.toThrow(
        'Account not found'
      );
    });
  });
  ...
  describe('Account Operations', () => {
    ...
    it('should get account signers', async () => {
      const mockSigners = [{ key: testKeypair.publicKey(), weight: 1, type: 'ed25519_public_key' }];
      mockServer.loadAccount.mockResolvedValue({
        accountId: () => testKeypair.publicKey(),
        signers: mockSigners,
      });
