...
describe('ExportService', () => {
  ...
  it('should generate a CSV report for a payroll batch', async () => {
    ...
    expect(output).toContain(
      'txHash,organizationPublicKey,employeeId,amount,assetCode,assetIssuer,status,memo,timestamp'
    );

    // Check if the first mocked transaction data is in the output
    expect(output).toContain(
      'hash123,GORG123,EMP456,500.00,USDC,GUSDC123,Success,February Salary,2024-02-23T12:00:00.000Z'
    );

    // Check if the second mocked transaction data is in the output
    expect(output).toContain(
      'hash456,GORG123,EMP457,750.00,USDC,GUSDC123,Success,February Salary Plus Bonus,2024-02-23T12:05:00.000Z'
    );
  });
});
